...
const { collectOsm, collectGoogle, baseHeaders, readJsonBody } = require('../../lib/server');
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    try {
      osm = await collectOsm(center, input.radiusM);
    ...
    const ranked = rankEntities([
      ...(osm.entities || []),
      ...(google.entities || [])
    ], center, input.keywords, input.minScore).slice(0, 1500);
